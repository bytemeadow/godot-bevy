...
```bash
devenv shell -- python3 examples/harness/capture.py --example platformer-2d --scenario devices --run input-devices-1 --timeout 180
```
...
```bash
devenv shell -- python3 examples/harness/capture.py --example platformer-2d --scenario input-replay --run input-replay-1 --timeout 60
```
...
The standard evidence leaves are `target/example-evidence/<run>/platformer-2d/<scenario>/`. Each requires every `frame-NNNNNN.physical_input.json` or `frame-NNNNNN.synthetic_input.json` from 1 through N, plus the [core evidence](../../harness/README.md). The committed manifests retain PNGs at frames 0 and N and check the prompt overlay structurally. They do not compare images.

Run the adapter tests and schema validation:

```bash
python3 -m unittest discover -s examples/harness/physical_input/tests -v
python3 -m unittest discover -s examples/harness/synthetic_input/tests -v
python3 examples/harness/capture_schema.py examples/platformer-2d/capture/devices.json
python3 examples/harness/capture_schema.py examples/platformer-2d/capture/input-replay.json
```
