...
def validate(value):
    ...
    if errors:
        return errors
    directions = set()
    devices = {"key": "keyboard", "mouse_button": "mouse", "joy_button": "controller", "joy_axis": "controller"}
    for index, step in enumerate(value["steps"]):
        expected = step["expected"]
        if "action" in expected:
            if expected["action"] not in value["actions"]:
                errors.append(f"steps[{index}]: action must be in actions")
        elif devices[expected["event"]] != step["device"]:
            errors.append(f"steps[{index}]: event does not belong to device kind")
        if expected.get("event") == "joy_axis":
            if expected["neutral_tolerance"] >= expected["threshold"]:
                errors.append(f"steps[{index}]: neutral tolerance must be below direction threshold")
            if expected["direction"] == 0:
                if expected["axis"] not in directions:
                    errors.append(f"steps[{index}]: neutral requires a preceding direction on this axis")
                directions.discard(expected["axis"])
            else:
                directions.add(expected["axis"])
    return errors
