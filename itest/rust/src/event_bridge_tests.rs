...
fn test_send_event_after_teardown_noop(ctx: &TestContext) -> godot::task::TaskHandle {
    ...
    godot::task::spawn(async move {
        ...
        assert_eq!(
            witness.load(SeqCst),
            0,
            "no delivery after teardown — the app's channel is gone, so send_event finds no sender"
        );
...
/// A never-initialized BevyApp node (get_app() == None) no-ops, no FFI panic.
#[itest(async)]
...
/// node-scoping — binding the specific node reaches that node's world, so a
/// single shared sender can't route both to one app.
#[itest(async)]
...
fn test_send_event_dict_payload(ctx: &TestContext) -> godot::task::TaskHandle {
    ...
    godot::task::spawn(async move {
        ...
        let mut bridge = bridge_node(&ctx_clone);
        bridge.call(
            "send_event",
            &[
                "damage".to_variant(),
...
/// GDScript `send_event` with a null payload fires a unit event.
#[itest(async)]
...
/// Unknown name and a payload the mapper rejects both drop, no FFI panic.
#[itest(async)]
...
/// Strict decode: a float where an `i64` is asked for is dropped; an int succeeds.
#[itest(async)]
...
/// 200 unknown-name calls: no panic, no delivery (the warner's power-of-two
/// decay is covered by the `RateLimitedWarner` unit tests).
#[itest(async)]
...
/// Also guards the name collision: if a `#[func] send_event` ever shadows this
/// method, the call below resolves to the private `GString`/`Variant` func and
...
/// node's `InstanceId` (not the `Gd`) and recovers it to make the nested call.
#[itest(async)]
    ...
    godot::task::spawn(async move {
        ...
        let mut app = TestApp::new(&ctx_clone, move |app| {
            ...
            app.add_godot_event::<EvA>("outer", move |_p| {
                if let Ok(mut node) = Gd::<godot::classes::Node>::try_from_instance_id(bevy_app_iid)
                {
...
/// A fire from inside a running frame (an `Update` system via
/// `Res<GodotEventSender>`) is NOT delivered that frame — this frame's `First`
/// drain already ran — and lands on the next `First` drain, never dropped.
#[itest(async)]
fn test_send_event_mid_frame_defers_to_next_frame(ctx: &TestContext) -> godot::task::TaskHandle {
    let ctx_clone = ctx.clone();
    godot::task::spawn(async move {
        #[derive(Resource, Default)]
        struct Received(i32);
        ...
        let mut app = TestApp::new(&ctx_clone, |app| {
            ...
            // Fires once, mid-frame, from Update (after this frame's First drain).
            app.add_systems(
                Update,
                |mut fire: ResMut<FireNow>, sender: Res<GodotEventSender>| {
                    if fire.0 {
                        sender.send(Damage { amount: 21 });
                        fire.0 = false;
                    }
                },
            );
        ...
        // Arm the fire between frames, then run one frame: the Update system sends
        // mid-frame, after First has already drained — so nothing is delivered yet.
        app.with_world_mut(|w| w.resource_mut::<FireNow>().0 = true);
        app.update().await;
        assert_eq!(
            app.with_world(|w| w.resource::<Received>().0),
            0,
            "a mid-frame fire is not delivered the same frame"
        );

        // The next frame's First drains it.
        app.update().await;
        assert_eq!(
            app.with_world(|w| w.resource::<Received>().0),
            21,
            "the deferred event is delivered on the next frame, never dropped"
        );
...
/// A cloned `GodotEventSender` sends from a worker thread; the main-thread
/// `First` drain delivers it. Proves the off-thread path is a plain channel
/// send — no bind, no thread hazard.
#[itest(async)]
fn test_send_event_off_thread_clone(ctx: &TestContext) -> godot::task::TaskHandle {
    let ctx_clone = ctx.clone();
    godot::task::spawn(async move {
        #[derive(Resource, Default)]
        struct Received(i32);
        ...
        // Clone on the main thread, move to a worker, fire there. Join before the
        // drain so the enqueue is guaranteed to precede the sampled frame.
        let sender = app.with_world(|w| w.resource::<GodotEventSender>().clone());
        std::thread::spawn(move || sender.send(Damage { amount: 33 }))
            .join()
        ...
        assert_eq!(
            app.with_world(|w| w.resource::<Received>().0),
            33,
            "off-thread cloned sender must deliver via the drain"
        );
