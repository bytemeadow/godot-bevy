...
impl TestApp {
        ...
        F: FnOnce(&mut App) + Send + 'static
    {
        ...
        // Wrap setup in Mutex so we can call FnOnce from Fn closure (thread-safe)
        let setup_mutex = Mutex::new(Some(setup));

        // Configure the app before initialization
        bevy_app.bind_mut().set_instance_init_func(Box::new(move |app: &mut App| {
            // Always add core plugins (schedules)
        ...
        C: Clone
    {
        self.with_world_mut(|world| {
            ...
            query.iter(world).next().expect("Should have entity").clone()
        })
    }
