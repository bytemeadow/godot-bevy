...
pub fn bevy_app(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        unsafe impl ExtensionLibrary for BevyExtensionLibrary {
            fn on_stage_init(level: godot::prelude::InitStage) {
                if level == godot::prelude::InitStage::Core {
            ...
            fn on_stage_deinit(_level: godot::prelude::InitStage) {
                if _level == godot::prelude::InitStage::Core {
