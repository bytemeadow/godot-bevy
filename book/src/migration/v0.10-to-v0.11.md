...
## Breaking: Signal connection APIs now take GodotAccess

Signal connection helpers now take `GodotAccess` and a `GodotNodeHandle`.
...
```rust,ignore
signals.connect(&mut godot, handle, "pressed");
```
...
- [ ] Update signal connection call sites to pass `GodotAccess`.
