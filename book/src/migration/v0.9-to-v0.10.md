...
- [Breaking: `connect_map` Callback Now Returns `Option<T>`](#breaking-connect_map-callback-now-returns-optiont)
...
```rust,ignore
typed.connect_map(&mut node, "signal", None, |_args, _node, _ent| MyEvent {});
// or for deferred:
TypedDeferredSignalConnections::<MyEvent>::with_connection("signal", |_a, _n, _e| MyEvent {});
```
**🟢 After (new API: return event value inside Some, or None to suppress):**
```rust,ignore
typed.connect_map(&mut node, "signal", None, |_args, _node, _ent| Some(MyEvent {}));
// or for deferred:
TypedDeferredSignalConnections::<MyEvent>::with_connection("signal", |_a, _n, _e| Some(MyEvent {}));
```
