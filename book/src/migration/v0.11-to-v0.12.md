...
- [Breaking: Detached nodes remain alive](#breaking-detached-nodes-remain-alive)
- [Behavior: Shutdown runs on destruction](#behavior-shutdown-runs-on-destruction)
- [Dependencies: Bevy 0.19 and godot 0.5](#dependencies-bevy-019-and-godot-05)
- [New: Event bridge](#new-event-bridge)
- [Breaking: Integration tests live in the game crate](#breaking-integration-tests-live-in-the-game-crate)
- [New: GodotNode on tuple structs](#new-godotnode-on-tuple-structs)
- [New: Inspector metadata on generated exports](#new-inspector-metadata-on-generated-exports)
- [New: AttachableComponent](#new-attachablecomponent)
- [Known limitations: Web builds](#known-limitations-web-builds)
...
- [Delta time: use Res\<Time\> everywhere](#replace-physicsdelta-with-restime)
- [Engine.time_scale honored automatically](#new-enginetime_scale-honored-automatically)
- [SceneTree.paused now runs Update systems (process_mode = ALWAYS)](#breaking-scenetreepaused-now-runs-update-systems-process_mode--always)
- [Physics interpolation support](#new-physics-interpolation-support)
- [Dependencies: gdext bumped to 0.5.5](#dependencies-gdext-bumped-to-055)
...
Follow [Bevy's 0.18 to 0.19 migration guide](https://bevy.org/learn/migration-guides/0-18-to-0-19/) and [gdext's migration to 0.5](https://godot-rust.github.io/book/migrate/v0.5.html) for changes in your own systems and Godot classes. The minimum gdext patch is 0.5.5, as described below.
...
If you used `GodotMailboxPlugin`, replace its field polling with event registration and explicit sends. Replace `MessageReader<T>` consumers with observers, or accumulate events in a resource when you need fixed-step batches. See [The Event Bridge](../project-transition/event-bridge.md) for the migration table and callback timing restrictions.
...
Configure the game runner and use `cargo run --features itest`. Direct Godot launches must set `GODOT_BEVY_ITEST=1` so the game autoload waits for the test runner. See [Testing](../testing/index.md) for dependencies, runner setup, and frame control.
...
Enable the opt-in `register-docs` feature to show descriptions in Godot's editor help. It requires Godot API 4.3 or later. On API 4.2, leave it disabled. Hints work without it, and descriptions remain Rust docs. See [Inspector metadata](../scene-tree/custom-nodes/property-mapping-with-bevy-bundle.md#inspector-metadata-for-generated-exports) for the attribute syntax.

---

## New: AttachableComponent

`AttachableComponent` turns an editor-authored child node into a component on its parent's Bevy entity (#265). Use it for configuration authored as child nodes. The carrier is consumed after successful attachment and never gets its own entity, so do not keep references to it or expect live Inspector synchronization. Existing components need no migration. See [Attachable Components](../scene-tree/custom-nodes/attachable-components.md) for conversion, placement, and lifetime rules.

---

## Known limitations: Web builds

The web/wasm example does not build on current nightly (#268). The library's web features compile, but web execution remains unverified for 0.12. If you ship to browsers, keep this upgrade blocked on your own end-to-end validation and follow [#268](https://github.com/bytemeadow/godot-bevy/issues/268) for the build failure.
...
Secondary Bevy `SubApp`s are rejected when the app is built. The driver runs `Main` directly and cannot extract or update a secondary app. If a plugin creates one, remove that plugin or move its logic into the main app.
