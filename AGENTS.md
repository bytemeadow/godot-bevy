...
The `autosync` system (`godot-bevy/src/autosync.rs`) automatically registers custom Godot node types and inserts their Bevy components when a matching node enters the scene tree. Register via `#[derive(GodotNode)]` (component-first, generates the Godot class) or `#[derive(BevyComponents)]` (Godot-first, annotates a user-written `GodotClass`). Both use the `#[bevy(...)]` attribute grammar.
