...
Tier-2 profiling is diagnostic-only and uses the optimized, symbolized `profiling`
Cargo profile:
...
Profiling artifacts live only under `target/profiles/`.

## Codegen

Per-Godot-version files are generated by `uv run python -m godot_bevy_codegen` from `godot_extension_api/*.json`: `src/interop/node_markers/`, `src/interop/signal_names/`, `src/plugins/scene_tree/node_type_checking/`, their dispatcher files, and the GDScript watcher versions in `addons/`. Regenerate when adding a Godot API version; never edit generated files by hand. The run re-dumps each `extension_api*.json` by invoking that version's Godot binary, so it needs every supported version on PATH (see the book's gdenv page) -- devenv only ships the single version the tests use.
...
The launcher (`examples/run_godot.rs`, via `cargo-godot-lib`) generates each example's gitignored `godot/rust.gdextension`; building the library alone does not create it. CI uses `xtask gdextension` to write the descriptor before importing and exporting.

Gotcha: running a project with `godot --path` uses the `.debug` entry even after a `--release` build. The default descriptor points `.debug` and `.release` at their respective target directories, so copy the release library over the debug path (or edit the descriptor) to measure release performance. This matters most for perf-test. The `itest` launcher points both entries at the profile it was built with.
