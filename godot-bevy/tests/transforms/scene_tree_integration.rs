//! Scene tree integration tests for transform sync
...
/// Test that nodes added to scene tree get entities with transforms
pub fn test_scene_tree_node_creates_entity_with_transform(
    ...
    // Create and add a Node3D to scene tree
    ...
    // Process scene tree events
    ctx.app.update();

    // Verify entity was created with correct transform
    ...
    for (handle, transform) in query.iter(world) {
        if handle.instance_id() == node.instance_id() {
            ...
            assert!((transform.translation.z - 300.0).abs() < 0.01);
            found = true;
            break;
        }
    }

    assert!(found, "Should create entity with Transform for Node3D");
...
/// Test Bevy to Godot sync with full scene tree integration
pub fn test_bevy_to_godot_sync_with_scene_tree(ctx: &mut BevyGodotTestContext) -> TestResult<()> {
    // Set up full integration
    let mut env = ctx.setup_full_integration();
    ...
    // Modify Bevy transform in a system
    ctx.app
