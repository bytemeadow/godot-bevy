...
'''
    ...
    def generate_type_checking_code(self, node_types, parent_map):
        ...
        categories = self.categorize_types_by_hierarchy(node_types, parent_map)
        
        # Test which Godot classes actually exist by trying to use them
        valid_types = self.filter_valid_godot_classes(node_types)
        
        # Update categories with only valid types
...
'''
    ...
    def fix_godot_class_name(self, class_name):
...
'''
        ...
        for node_type in sorted(types):
            fixed_name = self.fix_godot_class_name(node_type)
            content += f'''    if node.try_get::<godot::classes::{fixed_name}>().is_some() {{
...
'''
    ...
    def run(self):
        ...
        try:
            ...
            node_types, parent_map = self.load_and_parse_api()
