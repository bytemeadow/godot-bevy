...
# Generated for Godot version: Godot Engine v4.5.stable.official
...
# Handles 271 different Godot node types.
...
func _analyze_node_type(node: Node) -> String:
    ...
    if node is Node3D:
        ...
        if node is OpenXRCompositionLayer: return "OpenXRCompositionLayer"
        if node is OpenXRCompositionLayerCylinder: return "OpenXRCompositionLayerCylinder"
        if node is OpenXRCompositionLayerEquirect: return "OpenXRCompositionLayerEquirect"
        if node is OpenXRCompositionLayerQuad: return "OpenXRCompositionLayerQuad"
        ...
        if node is OpenXRVisibilityMask: return "OpenXRVisibilityMask"
        ...
        if node is XRBodyModifier3D: return "XRBodyModifier3D"
        if node is XRCamera3D: return "XRCamera3D"
        if node is XRController3D: return "XRController3D"
        if node is XRFaceModifier3D: return "XRFaceModifier3D"
        if node is XRHandModifier3D: return "XRHandModifier3D"
    ...
    # Check Node2D hierarchy (common in 2D games)
        ...
        if node is Parallax2D: return "Parallax2D"
    ...
    # Check Control hierarchy (UI elements)
        ...
        if node is EditorToaster: return "EditorToaster"
        ...
        if node is GraphFrame: return "GraphFrame"
        ...
        if node is OpenXRBindingModifierEditor: return "OpenXRBindingModifierEditor"
        if node is OpenXRInteractionProfileEditor: return "OpenXRInteractionProfileEditor"
        if node is OpenXRInteractionProfileEditorBase: return "OpenXRInteractionProfileEditorBase"
    ...
    elif node is StatusIndicator: return "StatusIndicator"
